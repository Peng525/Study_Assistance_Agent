...
if ($shouldInvalidateQa) {
    $reason = 'Business code, tests, or QA configuration changed after the previous check.'
    Set-DemoQaGatePending -ProjectRoot $root -Gate 'code-review' -Reason $reason
    Set-DemoQaGatePending -ProjectRoot $root -Gate 'test' -Reason $reason
    $messages.Add('Demo QA gates were reset to PENDING.')
}
...
if ($messages.Count -gt 0) {
    Write-HookJson (New-AdditionalContextOutput -EventName 'PostToolUse' -Context ("Demo post-change checks:`n`n" + ($messages -join "`n`n")))
}
